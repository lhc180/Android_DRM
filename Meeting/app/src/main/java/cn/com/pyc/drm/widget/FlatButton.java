...
public class FlatButton extends Button implements View.OnTouchListener {
	...
	@SuppressLint("ClickableViewAccessibility") public FlatButton(Context context) {
		super(context);
		init();
		this.setOnTouchListener(this);
	}

	@SuppressLint("ClickableViewAccessibility") public FlatButton(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	...
	@SuppressLint("ClickableViewAccessibility") public FlatButton(Context context, AttributeSet attrs, int defStyle) {
	...
	int mDefaultButtonColor = 0x3eadeb;
	int mDefaultShadowColor = 0xff3493c8;
	int mDefaultShadowHeight = 4;
	int mDefaultCornerRadius = 8;
	...
	private void init() {
		...
		if (resources == null)
			return;
		mButtonColor = mDefaultButtonColor;
		mShadowColor = mDefaultShadowColor;
		mShadowHeight = mDefaultShadowHeight;
		mCornerRadius = mDefaultCornerRadius;
	}
	...
	private void parseAttrs(Context context, AttributeSet attrs) {
		...
		for (int i = 0; i < typedArray.getIndexCount(); i++) {
			...
			if (attr == R.styleable.FlatButton_shadowEnabled) {
				isShadowEnabled = typedArray.getBoolean(attr, true); // Default
																		// is
																		// true
			} else if (attr == R.styleable.FlatButton_buttonColor) {
				mButtonColor = typedArray.getColor(attr, mDefaultButtonColor);
			} else if (attr == R.styleable.FlatButton_shadowColor) {
				mShadowColor = typedArray.getColor(attr, mDefaultShadowColor);
				isShadowColorDefined = true;
			} else if (attr == R.styleable.FlatButton_shadowHeight) {
				mShadowHeight = typedArray.getDimensionPixelSize(attr,
						mDefaultShadowHeight);
