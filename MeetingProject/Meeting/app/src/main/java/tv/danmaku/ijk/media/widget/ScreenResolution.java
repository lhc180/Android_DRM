...
/**
 * Class to get the real screen resolution includes the system status bar. We
 * can get the value by calling the getRealMetrics method if API >= 17
...
public class ScreenResolution {
	...
	/**
	 * Gets resolution on old devices. Tries the reflection to get the real
	 * resolution first. Fall back to getDisplayMetrics if the above method
	 * failed.
	 * */
	private static Pair<Integer, Integer> getRealResolutionOnOldDevice(
			Context ctx) {
		try {
			...
			return new Pair<Integer, Integer>(disp.widthPixels,
					disp.heightPixels);
		}
	}
	...
	private static Pair<Integer, Integer> getRealResolution(Context ctx) {
		...
		return new Pair<Integer, Integer>(metrics.widthPixels,
