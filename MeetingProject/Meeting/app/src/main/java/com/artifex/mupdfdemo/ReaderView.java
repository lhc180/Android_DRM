...
public class ReaderView extends AdapterView<Adapter> implements GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener, Runnable {
    ...
    public static boolean HORIZONTAL_SCROLLING = true;
...
//        AlphaAnimation aAnima = new AlphaAnimation(0.0f, 1.0f);
//        aAnima.setDuration(1000);
//        v.startAnimation(aAnima);
    }
    ...
    public void smartMoveForwards() {
        ...
        if (bottom >= docHeight) {
            ...
            if (right + screenWidth > docWidth) {
                ...
                DRMLog.e("mCurrent", mCurrent + 1 + "");
//                if (nv != null) {
//                    AlphaAnimation aAnima = new AlphaAnimation(0.0f, 1.0f);
//                    aAnima.setDuration(1000);
//                    nv.startAnimation(aAnima);
//                }
...
//                    AlphaAnimation aAnima = new AlphaAnimation(0.0f, 1.0f);
//                    aAnima.setDuration(1000);
//                    pv.startAnimation(aAnima);
//                }
                if (pv == null) /* No page to advance to */ return;
            ...
            switch (directionOfTravel(velocityX, velocityY)) {
                case MOVING_LEFT:
                    if (HORIZONTAL_SCROLLING && bounds.left >= 0) {
                        ...
                        if (vl != null) {
//                            AlphaAnimation aAnima = new AlphaAnimation(0.0f, 1.0f);
//                            aAnima.setDuration(5000);
//                            vl.startAnimation(aAnima);
...
//                            vl.startAnimation(aAnima);
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        if (mReflow) {
            View v = mChildViews.get(mCurrent);
            if (v != null) onScaleChild(v, mScale);
        } else {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (!mResetLayout) {
            ...
            if (cv != null) {
                ...
                if (HORIZONTAL_SCROLLING) move = cv.getLeft() + cv.getMeasuredWidth() + cvOffset.x + GAP / 2 + mXScroll < getWidth() / 2;
                else move = cv.getTop() + cv.getMeasuredHeight() + cvOffset.y + GAP / 2 + mYScroll < getHeight() / 2;
                ...
                if (HORIZONTAL_SCROLLING) move = cv.getLeft() - cvOffset.x - GAP / 2 + mXScroll >= getWidth() / 2;
                else move = cv.getTop() - cvOffset.y - GAP / 2 + mYScroll >= getHeight() / 2;
                if (move && mCurrent > 0) {
                    postUnsettle(cv);
                    // post to invoke test for end of animation
                    // where we must set hq area for the new current view
    ...
    public void setSelection(int arg0) {
        throw new UnsupportedOperationException("不被支持");
    }

    private View getCached() {
        if (mViewCache.size() == 0) return null;
        else return mViewCache.removeFirst();
    }
    ...
    private static int directionOfTravel(float vx, float vy) {
        if (Math.abs(vx) > 2 * Math.abs(vy)) return (vx > 0) ? MOVING_RIGHT : MOVING_LEFT;
        else if (Math.abs(vy) > 2 * Math.abs(vx)) return (vy > 0) ? MOVING_DOWN : MOVING_UP;
        else return MOVING_DIAGONALLY;
    }
