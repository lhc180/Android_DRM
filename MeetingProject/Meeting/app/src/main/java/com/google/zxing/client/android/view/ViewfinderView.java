...
public final class ViewfinderView extends View
{
	...
	public ViewfinderView(Context context, AttributeSet attrs)
	{
		...
		if (mBitmapLine == null)
			mBitmapLine = ((BitmapDrawable) getResources().getDrawable(
					R.drawable.qrcode_scan_line)).getBitmap();
	...
	public void onDraw(Canvas canvas)
	{
		try
		{
			...
			if (cameraManager == null) { return; // not ready yet, early draw
													// before
													// done configuring
			}
			...
			{ // 绘制扫描结果的图
				// Draw the opaque result bitmap over
		...
		{
			Log.e("ViewfinderView",
					"ViewfinderView  -> onDraw() Canvas: trying to use a recycled bitmap");
		}
	}
	...
	private void drawScanningLine(Canvas canvas, Rect frame)
	{
		...
		canvas.drawBitmap(mBitmapLine, null, lineRect, paint);
	}
	...
	{
		int color = Color.argb(255, 79, 193, 233);
		paint.setColor(color);
		// paint.setColor(Color.WHITE);
		paint.setAlpha(OPAQUE);

		canvas.drawRect(frame.left - 5, frame.top - 5,
				frame.left + frame.width() / 9, frame.top + 5, paint);
		canvas.drawRect(frame.left + (frame.width() * 8) / 9, frame.top - 5,
				frame.right, frame.top + 5, paint); // top

		canvas.drawRect(frame.left - 5, frame.top, frame.left + 5, frame.top
				+ frame.height() / 9, paint);
		canvas.drawRect(frame.left - 5, frame.top + (frame.height() * 8) / 9,
				frame.left + 5, frame.bottom, paint); // left

		canvas.drawRect(frame.right - 5, frame.top - 5, frame.right + 5,
				frame.top + frame.height() / 9, paint);
		canvas.drawRect(frame.right - 5, frame.top + (frame.height() * 8) / 9,
				frame.right + 5, frame.bottom, paint); // right

		canvas.drawRect(frame.left - 5, frame.bottom - 5,
				frame.left + frame.width() / 9, frame.bottom + 5, paint);
		canvas.drawRect(frame.left + (frame.width() * 8) / 9, frame.bottom - 5,
				frame.right + 5, frame.bottom + 5, paint); // bottom
		...
		// frame.top
		// + CORNER_PADDING, paint);
		// canvas.drawBitmap(bitmapCornerTopright,
		// frame.right - CORNER_PADDING - bitmapCornerTopright.getWidth(),
		// frame.top
		// + CORNER_PADDING, paint);
		// canvas.drawBitmap(bitmapCornerBottomLeft, frame.left +
		// CORNER_PADDING, 2 + (frame.bottom
		// - CORNER_PADDING - bitmapCornerBottomLeft.getHeight()), paint);
		// canvas.drawBitmap(bitmapCornerBottomRight, frame.right -
		// CORNER_PADDING
		// - bitmapCornerBottomRight.getWidth(),
		// 2 + (frame.bottom - CORNER_PADDING -
		// bitmapCornerBottomRight.getHeight()), paint);
		//
	...
	public void recyle()
	{
		if (!mBitmapLine.isRecycled() && mBitmapLine != null)
		{
			mBitmapLine = null;
		}
	}
