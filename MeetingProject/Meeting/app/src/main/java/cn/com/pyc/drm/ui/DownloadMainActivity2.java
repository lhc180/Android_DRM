...
import cn.com.pyc.drm.adapter.DownloadMainAdapter2;
 ...
 * 
 */
public class DownloadMainActivity2 extends BaseActivity implements OnClickListener, IXListViewListener {

	private String TAG = DownloadMainActivity2.class.getSimpleName();
	...
	private DownloadMainAdapter2 adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_download_main2);
		init_Value();
		init_View();
		init_Listener();
		load_Data();
...
//					Log.d("", "href: " + url);
	...
	public class JavaScriptStartDownloadActivity {
		...
		*/
		...
		public void downLoad() {
			mHandler.post(new Runnable() {
				public void run() {
					startActivity(new Intent(DownloadMainActivity2.this,DownloadMainActivity2.class));
				}
			});
		}
	}
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_NET_NONE: {
				...
				localAdapter = new DownloadLocalAdapter(DownloadMainActivity2.this, albumsLocals);
	...
	private BroadcastReceiver downloadReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent == null)
				return;
			if (context == null)
				context = DownloadMainActivity2.this;
			// 必传参数position
	...
	private void CollDownloadStatisticsWS(String topicname) {
		...
		String devicesid = DeviceUtils.getIMEI(DownloadMainActivity2.this);
...
//	}
		...
		if (obj != null && obj instanceof DownloadInfo) {
			...
			if (albm != null) {
				...
				if (TextUtils.isEmpty(publishDate) || TextUtils.equals(publishDate, savePublishDate)) {
					...
					int taskSize = DownloadMainActivity2.sTaskIdSet.size();
					if (taskSize >= Constant.sTaskCount) {
						showToast(DownloadMainActivity2.this.getString(R.string.please_waiting_n_update_download, taskSize));
						return;
					}
	...
	private void needDownload(final int position, final DownloadInfo o) {
		...
		if (sTaskIdSet.contains(myProId)) {
			...
			if (!CommonUtil.isWifi(DownloadMainActivity2.this)) {
				clearDlg = new NormalDialog().dialog(DownloadMainActivity2.this, "您正在使用非WiFi网络。", "是否继续下载?", new NormalDialogCallBack() {
	...
	public void onClick(View v) {
		...
		case R.id.txv_bottom_vote: {
			Intent intent = new Intent(DownloadMainActivity2.this, VoteActivity.class);
			...
			UIHelper.startInAnim(DownloadMainActivity2.this);
		}
			break;
	...
	private void requestData() {
		...
		if (TextUtils.isEmpty(Constant.getKey_mobile())) {
			actionUrl = actionUrl.substring(0,actionUrl.lastIndexOf("&SitePath"));
		}
		String memberid = (String) SPUtils.get(DRMUtil.KEY_PHONE_NUMBER, "");
...
//			Toast.makeText(getApplicationContext(), "没有加载到数据", Toast.LENGTH_SHORT).show();
		...
		if (adapter == null || currentPage == 1) {
			// 刷新或者第一次加载
			adapter = new DownloadMainAdapter2(this, oNetList);
		...
		XMLParserVersionManager.getInstance().checkExistDownloadData(infos, new OnCheckDownloadDataListener() {
			...
			public void onExist(boolean isSingleData, final int position, Object obj) {
				...
				if (isSingleData) {
					...
					// 下载
					ViewUtil.showContentDialog(DownloadMainActivity2.this, getString(R.string.download_1_task_to_handle, o.getProductName()), new OnTVAnimDialogClickListener() {
