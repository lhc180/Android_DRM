...
//			return "DRM";
	...
	public static String[] getHostAndPortByResult(String resultUrl)
	{
		...
		try
		{
			...
			if(-1==middleOffset){
				returnStrs[0] = newResult.replace("?","");
				returnStrs[1] = "";
				return returnStrs;
			}
	 ...
	 * 保存到sp中的host和port
	 */
	public static void SaveSP_HostPort(String ServerAddress) {// 切换时，更新端口号
		SPUtils.save(DRMUtil.SCAN_SERVERADDRESS, ServerAddress);
		String[] hostAndPortString = StringUtil.getHostAndPortByResult(ServerAddress);
	...
	/**
	 * 保存用户登陆后，选择机构的地址。保存在本地数据库中。
	 */
	public static void SaveLoginSelect_Mechanism_HostPort(String ServerAddress) {// 切换时，更新端口号
		String[] hostAndPortString = StringUtil.getHostAndPortByResult(ServerAddress);
		if (hostAndPortString != null) {
			// 保存主机名。eg： video.suizhi.net
			SPUtils.save(DRMUtil.SCAN_FOR_HOST, hostAndPortString[0]);
	...
	/**
	 * 保存到sp中下载专辑使用的host和port
	 */
	public static void SaveSP_Download_HostPort(String dataSource){
		// 根据返回的url获取主机和端口保存
		String[] hostAndPortString = StringUtil.getHostAndPortByResult(dataSource + "/");
		if (hostAndPortString != null) {
			// 保存主机名。eg： video.suizhi.net
			SPUtils.save(DRMUtil.Download_HOST, hostAndPortString[0]);
			// 保存端口号。eg：8657
			SPUtils.save(DRMUtil.Download_PORT, hostAndPortString[1]);
		}
	 ...
	 */
	public static boolean isMechanismQRcode(final String DataSource) {
		if (TextUtils.isEmpty(DataSource) ) {
			return false;//会议码,需要访问网络
		}
		return DataSource.contains("ServerName");//机构码,结构吗不需要访问网络,直接解析
//		final String GetCompany = StringUtil.getStringByColonHalf(DataSource, "?op", "=");
//		if (TextUtils.isEmpty(GetCompany) && !"ServerName".equals(GetCompany)) {
//			return false;
//		}
//		return true;
	}
