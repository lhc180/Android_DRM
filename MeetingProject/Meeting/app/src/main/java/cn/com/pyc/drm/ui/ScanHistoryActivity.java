...
public class ScanHistoryActivity extends BaseActivity {
    ...
    private static final int SCAN_HISTORY = 102;
	...
	protected void open(int position) {
		...
		final ScanHistory sh = listSH.get(position);
		StringUtil.SaveSP_Download_HostPort(sh.getScanDataSource());
		StringUtil.SaveSP_HostPort(sh.getVerify_url());
		 ...
		 * 2.查询会议是否需要第三方登陆
		 */
		WSHelper.newBuilder().setmUrl(Constant.getSynMeetingState_URL()).setMethodName(Constant.SynMeetingState_MethodName)
				.putParam("MeetingId", (String) SPUtils.get(DRMUtil.KEY_SUIZHI_CODE,""))
				.setCallback(new WSHelper.OnLoadDataListener<ReturnVerificationDataBean>() {
					@Override
					public void onSuccess(ReturnVerificationDataBean returnVerificationDataBean) {
						if(returnVerificationDataBean.getResult().equals("true")){
							StringUtil.SaveSP_HostPort(sh.getScanDataSource());
							getMeetingName(sh.getUsername(), sh.getMeetingId(), sh.getMeetingType(), sh.getScanDataSource());

						}else{
							showToast(returnVerificationDataBean.getMsg());
						}
					}
	...
	private void checkVersion() {
		...
		XMLParserVersionManager.getInstance().checkUpdate(this, isStop,new OnCheckResultListener() {
			@Override
