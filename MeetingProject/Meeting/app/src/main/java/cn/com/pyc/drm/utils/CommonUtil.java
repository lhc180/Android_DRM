...
//		sb.append((temp < 10) ? "0" + temp + "小时" : "" + temp + "小时");
	...
	public static void showExitDialog(final Activity mActivity){
		ViewUtil.showContentDialog(mActivity, "", new TVAnimDialog.OnTVAnimDialogClickListener() {
			@Override
			public void onClick(int dialogId) {
				if (dialogId == TVAnimDialog.DIALOG_CONFIRM) {
					DRMLog.i("Exit App");
					// 停止所有任务
					sendBroadcastStopAllTask(mActivity);
					stopMediaService(mActivity);
	 ...
	 * 发送广播，通知停止所有下载任务
	 */
	private static void sendBroadcastStopAllTask(Activity mActivity) {
	 ...
	 * 停止媒体服务
	 */
	private static void stopMediaService(Activity mActivity) {
		...
		// 清除音乐notification
		delMusicNotify(mActivity);
	}
	private static void delMusicNotify(Activity mActivity) {
		NotificationPatManager patManager = new NotificationPatManager(mActivity);
		patManager.cancelNotification();
	}



	public static List<MeetingBean> searchList(List<MeetingBean> old_searchList, String search_str) {
		if(old_searchList==null || old_searchList.size()==0)return null;
		List<MeetingBean> new_searchList = new ArrayList<MeetingBean>();
		for (int x = 0; x < old_searchList.size(); x++) {
			MeetingBean mb=old_searchList.get(x);
			String capital_search_str=search_str.toUpperCase();//转

			if(mb.getMeetingName()!=null && mb.getMeetingName().toUpperCase().indexOf(capital_search_str)>=0)
			{
				new_searchList.add(mb);
			}
		}
		return new_searchList;
	}
