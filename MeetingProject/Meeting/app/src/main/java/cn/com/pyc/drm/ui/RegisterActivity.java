...
public class RegisterActivity extends BaseActivity implements OnClickListener {
	...
	protected void setRegisterReturnCode(ReturnDataBean bm) {
		...
		if ("true".equals(bm.getResult())) {
			DialogUtil.showDialog_Prompt(mBaseActivity,"去登录","取消","注册成功,点击进入登陆界面?", new DialogUtil.onDialogPromptListener() {
				@Override
...
//		mbtn_verification_code.setTextColor(getResources().getColor(R.color.black));
		...
		new AsyncTask<String, String, String>() {
			protected String doInBackground(String... params) {
				String return_str = RequestHttpManager.getGetPhoneCode(params[0],"0");
				return return_str;
			}
