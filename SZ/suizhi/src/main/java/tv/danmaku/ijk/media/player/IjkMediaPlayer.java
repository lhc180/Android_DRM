...
/**
 * @author bbcallen Java wrapper of ffplay.
 */
public final class IjkMediaPlayer extends AbstractMediaPlayer
{
	 ...
	 * the media. Either a surface holder or surface must be set if a display or
	 * video sink is needed. Not calling this method or
	 * {@link #setSurface(Surface)} when playing back a video will result in
	 * only the audio track being played. A null surface holder or surface will
	 ...
	 * A null surface will result in only the audio track being played. If the
	 * Surface sends frames to a {@link SurfaceTexture}, the timestamps returned
	 * from {@link SurfaceTexture#getTimestamp()} will have an unspecified zero
	 * point. These timestamps cannot be directly compared between different
	 * media sources, different instances of the same media source, or multiple
	 * runs of the same program. The timestamp is normally monotonically
	 * increasing and is unaffected by time-of-day adjustments, but it is reset
			...
			throws IOException, IllegalArgumentException, SecurityException, IllegalStateException
	{
		...
		{
			setDataSource(uri.getPath(), null, 0);
			return;
	 ...
	 */
	@Override
	public void setDataSource(String path, byte[] key, long len) throws IOException,
			IllegalArgumentException, SecurityException, IllegalStateException
	{
		mDataSource = path;
		_setDataSource(path, null, null, null, key, len);
	}

	public void setDataSource(String path, String key) throws IOException,
			IllegalArgumentException, SecurityException, IllegalStateException
	{
		mDataSource = path;
		// String paths =
		// "/storage/sdcard0/Android/data/cdd1c554-b83c-461f-9acc-b23899ba816b_2.mp4";
		// String keys = "E24DB22E5BD621885D30CB9495F9EFD2";
		_setDataSource(path, null, null, key, null, 0);
		// _setDataSourceWithKey(paths, null, null, keys);
	}

	// private native void _setDataSourceWithKey(String path, String[] keys,
	// String[] values,
	// String waterMark) throws IOException, IllegalArgumentException,
	// SecurityException,
	...
	private native void _setDataSource(String path, String[] keys, String[] values,
			String suizhiKey, byte[] key, long len) throws IOException, IllegalArgumentException,
			SecurityException, IllegalStateException;

	private native void _setDataSourceFd(int fd) throws IOException, IllegalArgumentException,
			SecurityException, IllegalStateException;
	 ...
	 */
	@CalledByNative
	private static void postEventFromNative(Object weakThiz, int what, int arg1, int arg2,
			Object obj)
	{
	...
	private static String onControlResolveSegmentUrl(Object weakThiz, int segment)
	{
		...
		if (weakThiz == null || !(weakThiz instanceof WeakReference<?>)) return null;
	...
	private static boolean onNativeInvoke(Object weakThiz, int what, Bundle args)
	{
		...
		if (weakThiz == null || !(weakThiz instanceof WeakReference<?>)) return false;
		...
		public String onMediaCodecSelect(IMediaPlayer mp, String mimeType, int profile, int level)
		{
			...
			for (int i = 0; i < numCodecs; i++)
			{
				...
				for (String type : types)
				{
					...
					Log.i(TAG,
							String.format(Locale.US, "candidate codec: %s rank=%d",
			...
			Log.i(TAG,
					String.format(Locale.US, "selected codec: %s rank=%d",
