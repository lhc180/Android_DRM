...
public class MediaPlayerProxy implements IMediaPlayer
{
	...
	@Override
	public void setDataSource(String path, byte[] key, long len)
			throws IOException, IllegalArgumentException, SecurityException, IllegalStateException
	{
		mBackEndMediaPlayer.setDataSource(path, key, len);
	}
		...
		if (listener != null)
		{
			...
			mBackEndMediaPlayer.setOnVideoSizeChangedListener(new OnVideoSizeChangedListener()
			{
				...
				public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sar_num,
						int sar_den)
				{
					finalListener.onVideoSizeChanged(MediaPlayerProxy.this, width, height, sar_num,
							sar_den);
				}
			});
		}
