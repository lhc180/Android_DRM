...
public class SmoothCheckBox extends View implements Checkable
{
	...
	private static final int COLOR_TICK = Color.WHITE;									//默认对勾颜色
	private static final int COLOR_UNCHECKED = Color.WHITE;								//默认未选中颜色
	private static final int COLOR_CHECKED = Color.parseColor("#FB4846");				//默认选中颜色
	private static final int COLOR_FLOOR_UNCHECKED = Color.parseColor("#DFDFDF");		//默认未选中边框颜色
	private static final int DEF_DRAW_SIZE = 25;										//默认大小
	private static final int DEF_ANIM_DURATION = 300;									//默认动画的时长
	...
	private void init(AttributeSet attrs)
	{
		...
		mFloorColor = ta.getColor(R.styleable.SmoothCheckBox_scb_color_unchecked_stroke,
		...
		mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_scb_stroke_width,
				CommonUtil.dip2px(getContext(), 0));
		ta.recycle();
	...
	protected Parcelable onSaveInstanceState()
	{
		...
		bundle.putBoolean(KEY_CHECK_STATE, isChecked());
		return bundle;
	}
	...
	protected void onRestoreInstanceState(Parcelable state)
	{
		if (state instanceof Bundle)
		{
			...
			boolean isChecked = bundle.getBoolean(KEY_CHECK_STATE, false);
	...
	{
		int defSize = CommonUtil.dip2px(getContext(), DEF_DRAW_SIZE);
	...
	private void drawTickPath(Canvas canvas)
	{
		...
		if (mDrewDistance < mLeftLineDistance)
		{
			...
			mDrewDistance += step;
			float stopX = mTickPoints[0].x + (mTickPoints[1].x - mTickPoints[0].x) * mDrewDistance
					/ mLeftLineDistance;
			float stopY = mTickPoints[0].y + (mTickPoints[1].y - mTickPoints[0].y) * mDrewDistance
					/ mLeftLineDistance;
		...
		else
		{
			...
			{
				float stopX = mTickPoints[1].x + (mTickPoints[2].x - mTickPoints[1].x)
						* (mDrewDistance - mLeftLineDistance) / mRightLineDistance;
				float stopY = mTickPoints[1].y - (mTickPoints[1].y - mTickPoints[2].y)
						* (mDrewDistance - mLeftLineDistance) / mRightLineDistance;
