...
/**
 * A pratice demo use GradientDrawable to realize the effect of JakeWharton's
 * CirclePageIndicator
 */
public class RoundCornerIndicaor extends View implements PageIndicator
{
			...
			int defStyleAttr)
	{
		...
		TypedArray ta = context.obtainStyledAttributes(attrs,
				R.styleable.RoundCornerIndicaor);
		indicatorWidth = ta.getDimensionPixelSize(
				R.styleable.RoundCornerIndicaor_rci_width, dp2px(6));
		indicatorHeight = ta.getDimensionPixelSize(
				R.styleable.RoundCornerIndicaor_rci_height, dp2px(6));
		indicatorGap = ta.getDimensionPixelSize(
				R.styleable.RoundCornerIndicaor_rci_gap, dp2px(8));
		cornerRadius = ta.getDimensionPixelSize(
				R.styleable.RoundCornerIndicaor_rci_cornerRadius, dp2px(3));
		strokeWidth = ta.getDimensionPixelSize(
				R.styleable.RoundCornerIndicaor_rci_strokeWidth, dp2px(0));
		selectColor = ta.getColor(
				R.styleable.RoundCornerIndicaor_rci_selectColor,
				Color.parseColor("#ffffff"));
		unselectColor = ta.getColor(
				R.styleable.RoundCornerIndicaor_rci_unselectColor,
				Color.parseColor("#88ffffff"));
		strokeColor = ta.getColor(
				R.styleable.RoundCornerIndicaor_rci_strokeColor,
				Color.parseColor("#ffffff"));
		isSnap = ta.getBoolean(R.styleable.RoundCornerIndicaor_rci_isSnap,
				false);
	...
	{
		if (vp == null) { throw new IllegalStateException(
				"ViewPager can not be NULL!"); }

		if (vp.getAdapter() == null) { throw new IllegalStateException(
				"ViewPager adapter can not be NULL!"); }
			...
			int positionOffsetPixels)
	{
		...
		{
			/**
			 * position:当前View的位置 positionOffset:当前View的偏移量比例.[0,1)
			 */
	...
	/**
	 * 在ViewPager.OnPageChangeListener的onPageSelected方法中调用,不要同时调用onPageScrolled,
	 * 会出现闪烁情况
	 */
	...
	protected void onDraw(Canvas canvas)
	{
		...
		int verticalOffset = getPaddingTop()
				...
				- indicatorHeight / 2;
		int indicatorLayoutWidth = indicatorWidth * count + indicatorGap
				* (count - 1);
		int horizontalOffset = getPaddingLeft()
				+ (getWidth() - getPaddingLeft() - getPaddingRight()) / 2
				- indicatorLayoutWidth / 2;
	...
	private void drawSelect(Canvas canvas, int verticalOffset,
			int horizontalOffset)
	{
		int delta = (int) ((indicatorGap + indicatorWidth) * (isSnap ? 0
				: positionOffset));

		selectRect.left = horizontalOffset + (indicatorWidth + indicatorGap)
				* currentItem + delta;
	...
	private int measureWidth(int widthMeasureSpec)
	{
		...
		} else
		{
			...
			result = padding + indicatorWidth * count + indicatorGap
					* (count - 1);
