...
public class AppUpdateService extends Service {
     ...
     * 下载apk的名称
     */
    private final String APK_NAME = "suizhi";
     ...
     */
    public static void openAppUpdateService(Context ctx, String downloadUrl, String newVersion) {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null) {
            ...
            DRMLog.d("downloadUrl=" + downloadUrl);
        ...
        // apk下载保存地址
        destDir = new File(PathUtil.getDefaultSDCardRootPath());
        if (!destDir.exists())
            destDir.mkdirs();
        if (destDir.exists()) {
        ...
        mPendingIntent = PendingIntent.getActivity(AppUpdateService.this, R.string.app_name,
                completingIntent, PendingIntent.FLAG_UPDATE_CURRENT);

        mNotification.icon = R.drawable.ic_launchers;
        mNotification.tickerText = "开始下载...";
        mNotification.contentIntent = mPendingIntent;
        mNotification.contentView.setProgressBar(R.id.app_upgrade_progressbar, 100, 0, false);
        mNotification.contentView.setTextViewText(R.id.app_upgrade_title, getString(R.string
                .app_name) + updateVersion);
        mNotification.contentView.setTextViewText(R.id.app_upgrade_progresstext, "0%");
        mNotificationManager.cancel(mNotificationId);
        mNotificationManager.notify(mNotificationId, mNotification);
        // 通知设置页面的progressbar显示
        EventBus.getDefault().post(new UpdateBarEvent(true, BaseEvent.Type.UPDATE_SETTING_BAR));
    ...
    private DownloadUtils.DownloadListener downloadListener = new DownloadUtils.DownloadListener() {
        ...
        public void downloading(int progress) {
            if (progress > mProgress) {
                mNotification.contentView.setProgressBar(R.id.app_upgrade_progressbar, 100,
                        progress, false);
        ...
        public void downloaded() {
            mPendingIntent = null;
            mPendingIntent = PendingIntent.getActivity(AppUpdateService.this, R.string.app_name,
                    installIntent(destFile), PendingIntent.FLAG_UPDATE_CURRENT);
            mNotification.contentView.setViewVisibility(R.id.app_upgrade_progressblock, View
                    .VISIBLE);
            ...
            mNotificationManager.notify(mNotificationId, mNotification);
            // 下载完成，通知设置页面的bar隐藏
            EventBus.getDefault().post(new UpdateBarEvent(false, BaseEvent.Type.UPDATE_SETTING_BAR));
        }
    };
    ...
    public boolean checkApkFile(String apkFilePath) {
        ...
        try {
            ...
            PackageInfo pInfo = pManager.getPackageArchiveInfo(apkFilePath, PackageManager
                    .GET_ACTIVITIES);
            if (pInfo == null) {
                result = false;
            } else {
