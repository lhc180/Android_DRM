...
/**
 * 根据 CPU核数、时钟频率 以及 内存大小 对设备进行分级,(不同的年份手机产品，进行不同的操作)
 * 
 * @author hudq
 * 
 */
public class DeviceYearClassUtils
{
	// Year definitions
	 ...
	 * <br/>
	 * 
	 * if (year >= 2013) { // Do advanced animation } else if (year > 2010) { //
	 * Do simple animation } else { // Phone too slow, don't do any animations }
	 */
	public static int get(Context c)
	{
		if (mYearCategory == null)
		{
			synchronized (DeviceYearClassUtils.class)
			{
				if (mYearCategory == null)
				{
	...
	/**
	 * Calculates the "best-in-class year" of the device. This represents the
	 * top-end or flagship devices of that year, not the actual release year of
	 * the phone. For example, the Galaxy Duos S was released in 2012, but its
	 * specs are very similar to the Galaxy S that was released in 2010 as a
	 * then top-of-the-line phone, so it is a 2010 device.
	 * 
	 * @return The year when this device would have been considered
	 *         top-of-the-line.
	 */
	 ...
	 * </table>
	 * 
	 * @return the year in which top-of-the-line phones had the same number of
	 *         processors as this phone.
	 */
	private static int getNumCoresYear()
	{
		int cores = DeviceUtils.getCPUCoresNum();
		if (cores < 1)
			return CLASS_UNKNOWN;
		if (cores == 1)
			return CLASS_2008;
	 ...
	 * </table>
	 * 
	 * @return the year in which top-of-the-line phones had the same clock
	 *         speed.
	 */
	private static int getClockSpeedYear()
	{
		long clockSpeedKHz = DeviceUtils.getCPUMaxFreqKHz();
		if (clockSpeedKHz == DeviceUtils.DEVICEINFO_UNKNOWN)
			return CLASS_UNKNOWN;
		// These cut-offs include 20MHz of "slop" because my "1.5GHz" Galaxy S3
		// reports
		// its clock speed as 1512000. So we add a little slop to keep things
		// nominally correct.
	...
	/**
	 * Calculates the year class by the amount of RAM the phone has. Evaluations
	 * are based off the table below:
	 * <table border="1">
	 * <thead>
	 * <tr>
	 * <th width="50%">Amount</th>
	 ...
	 * </table>
	 * 
	 * @return the year in which top-of-the-line phones had the same amount of
	 *         RAM as this phone.
	 */
	private static int getRamYear(Context c)
	{
		long totalRam = DeviceUtils.getTotalMemory(c);
		if (totalRam <= 0)
			return CLASS_UNKNOWN;
		if (totalRam <= 192 * MB)
			return CLASS_2008;
