...
public class BitmapManager {
     ...
     * @param imageView
     */
    public static void loadBitmap(App context, String url, ImageView imageView, Bitmap defaultBmp) {
        ...
        if (bitmap != null) {
            ...
            String filepath = PathUtil.DEF_IMAGE_PATH + File.separator + filename;
            ...
            if (file.exists()) {
                ...
                queueJob(context, url, imageView, 0, 0);
            }
        }
    }
     ...
     * @param imageView
     */
    public static void loadMusicBitmap(App context, String url,
     ...
     * @param height
     */
    public static void queueJob(final App context, final String url,
                ...
                if (tag != null && tag.equals(url)) {
                    if (msg.obj != null) {
                        ...
                        ImageUtils.saveBitmap(
                                FileUtils.getNameFromFilePath(url),
                                (Bitmap) msg.obj, true);
                    }
                }
            }
     ...
     * @param type
     */
    public static void queueJobMusicBackGround(final App context,
                                               final String url, final int width, final int
                                                       height, final int type) {
        pool.execute(new Runnable() {
            public void run() {
    ...
    private static Map<String, SoftReference<Bitmap>> cacheBitmaps = new HashMap<String,
            SoftReference<Bitmap>>();

    /**
     * 计算图片的最小缩放比例
     *
     * @param options   图片option
     ...
     * @param height
     */
    private static Bitmap downloadBitmap(App context, String url, int width,
                                         int height) {
        Bitmap bitmap = null;
        try {
            // http加载图片
