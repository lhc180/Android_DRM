...
public class JPushReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        DRMLog.d(intent.getAction() + "\nextras: " + printBundle(bundle));
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            String regId = bundle
                    .getString(JPushInterface.EXTRA_REGISTRATION_ID);
            DRMLog.d("接收Registration Id : " + regId);
            // send the Registration Id to your server...

        } else if (JPushInterface.ACTION_MESSAGE_RECEIVED.equals(intent
                .getAction())) {
            DRMLog.d("接收到推送下来的自定义消息: "
            ...
            DRMLog.d("用户点击打开了通知: " + msg);

            openApp(context);

        } else if (JPushInterface.ACTION_RICHPUSH_CALLBACK.equals(intent
                .getAction())) {
            DRMLog.d("用户收到到RICH PUSH CALLBACK: "
            ...
            boolean connected = intent.getBooleanExtra(JPushInterface.EXTRA_CONNECTION_CHANGE,
                    false);
            DRMLog.w(intent.getAction() + " connected state change to " + connected);
            if (connected) {
                Constant.initDeviceId(context);
            }
        } else {
            DRMLog.d("Unhandled intent - " + intent.getAction());
        }
    }
