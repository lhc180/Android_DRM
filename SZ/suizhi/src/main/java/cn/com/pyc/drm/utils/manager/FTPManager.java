...
public class FTPManager
{
	 ...
	 * @return 是否连接成功
	 */
	public synchronized boolean connect(FTPClient ftpClient, String hostname,
			int port, String username, String password)
	{
		try
		{
			ftpClient.connect(hostname, port);
			DRMLog.d(TAG, "host: " + hostname);
			// DRMLog.d(TAG, "port: " + port);
	 ...
	 * @throws java.io.IOException
	 */
	public int upload(FTPClient ftpClient, String local, String remote)
			throws IOException
	{
		// 设置PassiveMode传输
		ftpClient.enterLocalPassiveMode();
		...
		if (remote.contains("/"))
		{
			...
			if (createDirecroty(remote, ftpClient) == UploadStatus.Create_Directory_Fail) { return UploadStatus.Create_Directory_Fail; }
		}
		...
		if (files.length == 1)
		{
			...
			{
				if (!ftpClient.deleteFile(remoteFileName)) { return UploadStatus.Delete_Remote_Faild; }
