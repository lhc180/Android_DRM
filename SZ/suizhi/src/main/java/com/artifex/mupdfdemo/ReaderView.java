	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom)
	{
		...
		if (mCurrent > 0)
		{
			...
			if (HORIZONTAL_SCROLLING)
			{
				...
				lv.layout(cvLeft - lv.getMeasuredWidth() - gap,
						(cvBottom + cvTop - lv.getMeasuredHeight()) / 2, cvLeft - gap, (cvBottom
		...
		if (mCurrent + 1 < mAdapter.getCount())
		{
			...
			if (HORIZONTAL_SCROLLING)
			{
				...
				rv.layout(cvRight + gap, (cvBottom + cvTop - rv.getMeasuredHeight()) / 2, cvRight
			...
			else
			{
				...
				rv.layout((cvLeft + cvRight - rv.getMeasuredWidth()) / 2, cvBottom + gap, (cvLeft
						+ cvRight + rv.getMeasuredWidth()) / 2,
...
//			}
	...
	private Rect getScrollBounds(View v)
	{
		...
		return getScrollBounds(v.getLeft() + mXScroll, v.getTop() + mYScroll,
				v.getLeft() + v.getMeasuredWidth() + mXScroll, v.getTop() + v.getMeasuredHeight()
						+ mYScroll);
	}

	private Point getCorrection(Rect bounds)
	{
		return new Point(Math.min(Math.max(0, bounds.left), bounds.right), Math.min(
				Math.max(0, bounds.top), bounds.bottom));
	}
